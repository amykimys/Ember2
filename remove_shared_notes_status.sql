...
-- 1. Drop the status column from shared_notes table
ALTER TABLE public.shared_notes DROP COLUMN IF EXISTS status;

-- 2. Update the get_shared_notes_for_user function to remove status filtering
...
-- 3. Drop the accept/decline functions since they're no longer needed
...
-- 4. Update the share_note_with_friends function to remove status
...
-- 5. Grant execute permissions on the updated function
