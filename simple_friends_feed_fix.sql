-- Simple Friends Feed Fix - Bulletproof Solution
-- This approach will definitely work by using a simpler, more direct method

-- Drop the existing function
DROP FUNCTION IF EXISTS public.get_friends_photo_shares_with_privacy(current_user_id uuid, limit_count integer);

-- Create a simple, reliable function that definitely fetches event titles
...
SELECT '=== TESTING SIMPLE FIX ===' as info;
SELECT 
    update_id,
    user_name,
    user_username,
...
-- Show a direct comparison of what should be fetched
SELECT '=== DIRECT EVENT TITLE FETCH ===' as info;
SELECT 
    su.id,
    su.source_type,
    su.source_id,
    su.caption,
    (SELECT title FROM events WHERE id = su.source_id::text LIMIT 1) as direct_event_title
