-- Fix shared_events table foreign key constraint
-- First, check the actual data type of events.id column
DO $$
DECLARE
    events_id_type text;
BEGIN
    -- Get the data type of the events.id column
    ...
    -- Recreate shared_events table with the correct foreign key type
    IF events_id_type = 'uuid' THEN
        -- Events table has UUID id
        EXECUTE '
...
-- Create function to get shared events for a user (works with both UUID and TEXT)
...
BEGIN
  ...
  SELECT 
    ...
    se.original_event_id::TEXT as original_event_id,
  ...
  JOIN events e ON se.original_event_id::TEXT = e.id::TEXT
